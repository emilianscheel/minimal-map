...
export function validateOpeningHoursStep(form: LocationFormState): FieldErrors {
	...
	for (const dayKey of OPENING_HOURS_DAY_ORDER) {
		...
		if ((hasOpen && !hasClose) || (!hasOpen && hasClose)) {
			dayErrors[dayKey] = __('Enter both opening and closing times.', 'emilian-scheel-minimal-map');
			continue;
		}

		if ((hasOpen && !isValidOpeningHoursTime(day.open)) || (hasClose && !isValidOpeningHoursTime(day.close))) {
			dayErrors[dayKey] = __('Enter valid opening hours in HH:MM format.', 'emilian-scheel-minimal-map');
			continue;
		}

		if (!hasOpeningHoursForDay(day) && hasLunchBreakForDay(day)) {
			dayErrors[dayKey] = __('Lunch breaks require opening hours for that day.', 'emilian-scheel-minimal-map');
			continue;
		}

		if ((hasLunchStart && !hasLunchDuration) || (!hasLunchStart && hasLunchDuration)) {
			dayErrors[dayKey] = __('Enter both lunch break start time and duration.', 'emilian-scheel-minimal-map');
			continue;
		}

		if (hasLunchStart && !isValidOpeningHoursTime(day.lunch_start)) {
			dayErrors[dayKey] = __('Enter a valid lunch break start time in HH:MM format.', 'emilian-scheel-minimal-map');
		}
	}
