...
export function useMarkersController(
	...
	const onConfirmEditMarkerColors = useCallback(async (): Promise<void> => {
		...
		try {
			...
			setActionNotice({
				status: 'success',
				message: __('Marker updated.', 'emilian-scheel-minimal-map'),
			});
		} catch (error) {
			setColorEditSubmitError(
				error instanceof Error ? error.message : __('Marker could not be updated.', 'emilian-scheel-minimal-map')
			);
		} finally {
			setSubmitting(false);
		}
	...
	const onDeleteMarker = useCallback(
		async (marker: MarkerRecord): Promise<void> => {
			...
			try {
				...
				setActionNotice({
					status: 'success',
					message: __('Marker deleted.', 'emilian-scheel-minimal-map'),
				});
			} catch (error) {
				setActionNotice({
					status: 'error',
					message:
						error instanceof Error
							? error.message
							: __('Marker could not be deleted.', 'emilian-scheel-minimal-map'),
				});
				throw error;
			} finally {
	...
	const onDeleteAllMarkers = useCallback(async (): Promise<void> => {
		...
		try {
			...
			setActionNotice({
				status: 'success',
				message: sprintf(
					_n('%d marker deleted.', '%d markers deleted.', allMarkers.length, 'emilian-scheel-minimal-map'),
					allMarkers.length
				),
			});
		} catch (error) {
			setActionNotice({
				...
				message:
					error instanceof Error
						? error.message
						: __('Markers could not be deleted.', 'emilian-scheel-minimal-map'),
			});
			throw error;
		} finally {
		...
		async (files: FileList | File[]): Promise<void> => {
			...
			try {
				...
				setActionNotice({
					...
					message:
						fileList.length === 1
							? __('Marker uploaded.', 'emilian-scheel-minimal-map')
							: sprintf(
								_n( '%d marker uploaded.', '%d markers uploaded.', fileList.length, 'emilian-scheel-minimal-map' ),
								fileList.length
							),
				});
			} catch (error) {
				setActionNotice({
					...
					message:
						error instanceof Error
							? error.message
							: __('Markers could not be uploaded.', 'emilian-scheel-minimal-map'),
				});
			} finally {
				setUploading(false);
			}
		...
		headerAction: enabled ? (
			<div className="minimal-map-admin__header-actions-group">
				<Button
					...
					label={__('Delete all markers', 'emilian-scheel-minimal-map')}
				...
				<Button
					...
					label={__('Marker library', 'emilian-scheel-minimal-map')}
