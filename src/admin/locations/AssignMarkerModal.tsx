...
export default function AssignMarkerModal({ controller }: { controller: LocationsController }) {
	...
	const options = controller.markers
		...
		.map((marker) => ({
			label: marker.title || __('Untitled marker', 'emilian-scheel-minimal-map'),
			value: `${marker.id}`,
		}));
	...
	return (
		<Modal
			...
			title={__('Assign Marker', 'emilian-scheel-minimal-map')}
			...
			<div className="minimal-map-admin__assign-to-collection-dialog">
				<div className="minimal-map-admin__assign-to-collection-copy">
					{isBulk ? (
						...
						<p className="minimal-map-admin__assign-to-collection-empty">
							{__('No custom marker assigned yet.', 'emilian-scheel-minimal-map')}
						</p>
					)}
				</div>
				...
				<ComboboxControl
					...
					label={__('Marker', 'emilian-scheel-minimal-map')}
					value={controller.assignmentMarkerId}
					options={options}
					onChange={(value) => controller.onSelectAssignmentMarker(value ?? '')}
					help={
						controller.markers.length === 0
							? __('Upload a marker first to assign one to these locations.', 'emilian-scheel-minimal-map')
							: options.length === 0
								? __('All available markers are already assigned for this selection.', 'emilian-scheel-minimal-map')
								: __('Choose a custom SVG marker to apply to the selected locations.', 'emilian-scheel-minimal-map')
					}
				/>
				...
				<div className="minimal-map-admin__assign-to-collection-actions">
					...
					>
						<span className="minimal-map-admin__location-dialog-button-content">
							<span>{__('Save Marker', 'emilian-scheel-minimal-map')}</span>
