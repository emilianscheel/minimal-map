...
export function useAnalyticsController(
			...
			.catch((error) => {
				...
				setLoadError(
					error instanceof Error
						? error.message
						: __('Analytics data could not be loaded.', 'emilian-scheel-minimal-map')
				);
			})
			.finally(() => {
				if (isMounted) {
		...
		try {
			...
			setNotice({
				...
				message:
					error instanceof Error
						? error.message
						: __('Analytics settings could not be updated.', 'emilian-scheel-minimal-map'),
			});
		} finally {
	...
	const onDeleteAllAnalytics = useCallback(async () => {
		...
		try {
			...
			setNotice({
				status: 'success',
				message: __('All tracking data deleted.', 'emilian-scheel-minimal-map'),
			});
		} catch (error) {
			setNotice({
				status: 'error',
				message:
					error instanceof Error
						? error.message
						: __('Analytics tracking data could not be deleted.', 'emilian-scheel-minimal-map'),
			});
		} finally {
